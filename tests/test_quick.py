...
class TestPayloadLoading:
    ...
    def test_sensitive_info_has_payloads(self, si_module) -> None:
        assert len(si_module.get_payloads()) >= 3

    def test_output_handling_has_payloads(self, oh_module) -> None:
        assert len(oh_module.get_payloads()) >= 3

    def test_system_prompt_has_payloads(self, sp_module) -> None:
        assert len(sp_module.get_payloads()) >= 3

    def test_payload_ids_unique(self, pi_module) -> None:
        payloads = pi_module.get_payloads()
        ids = [p.id for p in payloads]
        assert len(ids) == len(set(ids)), "Duplicate payload IDs detected"
...
class TestPromptInjection:
    ...
    def test_analyze_response(self, pi_module, response: str, expected_vulnerable: bool) -> None:
        ...
        assert result.vulnerable == expected_vulnerable, (
            ...
            f"expected vulnerable={expected_vulnerable}, got {result.vulnerable}"
        )

    def test_vulnerable_result_is_critical(self, pi_module) -> None:
        payload = pi_module.get_payloads()[0]
        result = pi_module.analyze_response(payload, "DAN Mode Enabled")
        assert result.severity == SeverityLevel.CRITICAL

    def test_safe_result_is_info(self, pi_module) -> None:
        payload = pi_module.get_payloads()[0]
        result = pi_module.analyze_response(payload, "I cannot do that.")
        assert result.severity == SeverityLevel.INFO

    def test_run_with_mock(self, fake_llm, pi_module) -> None:
...
class TestSensitiveInfo:
    ...
    def test_email_detected(self, si_module) -> None:
        payload = si_module.get_payloads()[0]
...
class TestOutputHandling:
    ...
    def test_pickle_loads_detected(self, oh_module) -> None:
        payload = oh_module.get_payloads()[-1]
        result = oh_module.analyze_response(payload, "pickle.loads(data)")
        assert result.vulnerable

    def test_warning_lowers_severity(self, oh_module) -> None:
        payload = oh_module.get_payloads()[0]
    ...
    def test_clean_code_safe(self, oh_module) -> None:
        payload = oh_module.get_payloads()[0]
...
class TestSystemPromptLeakage:
    ...
    def test_leakage_pattern_detected(self, sp_module) -> None:
        payload = sp_module.get_payloads()[0]
        result = sp_module.analyze_response(
...
class TestResponseAnalyzer:
    def _run_pi_module(self, fake_llm):
        fake_llm.send.return_value = "DAN Mode Enabled"
        return PromptInjectionModule(fake_llm).run()

    def test_summary_has_required_keys(self, fake_llm) -> None:
    ...
    def test_risk_score_nonzero_when_vulnerable(self, fake_llm) -> None:
        analyzer = ResponseAnalyzer()
        analyzer.add_results(self._run_pi_module(fake_llm))
        assert analyzer.build_summary()["risk_score"] > 0

    def test_clear_resets(self, fake_llm) -> None:
        analyzer = ResponseAnalyzer()
        analyzer.add_results(self._run_pi_module(fake_llm))
        analyzer.clear()
...
def test_all_modules_with_mock() -> None:
    ...
    for label, ModuleCls, response in cases:
        fake_llm.send.return_value = response
        results = ModuleCls(fake_llm).run()
        assert any(r.vulnerable for r in results), (
            f"Module '{label}': expected at least one vulnerable result"
        )


if __name__ == "__main__":
    test_all_modules_with_mock()
